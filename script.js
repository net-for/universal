...
function loginAttempt(){
    ...
    if(!password || password.length < 6){
        return showError('sheiyvane paroli');
    }
...
function registerAttempt(){
    ...
    if(!mail || mail.length < 3 || reg.test(mail) == false){
        return showError('sheiyvane swori email');
    }
    let pass_r =/^[A-Za-z0-9]{6,18}$/;
    if(pass_r.test(password) == false)
        return showError('paroli unda iyos latinuri asoebisa da ciprebisgan 6-18 simbolo')

    if(password != passwordConfirm){
        return showError('parolebi ar emtxveva');
    }

    if(gender_male.checked == false && gender_female.checked == false)
        return showError('airchie sxeuli');


    if(getSkin == -1)
        return showError('airchie skini');
